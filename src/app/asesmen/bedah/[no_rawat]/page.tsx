...
export default function AsesmenBedahPage({ params }: { params: Promise<{ no_rawat: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const assessmentRes = await fetch(`${serviceUrl}/penilaian-medis-bedah/${resolvedParams.no_rawat}`)
  ...
  const handleSave = async () => {
    ...
    try {
      const serviceUrl = process.env.NEXT_PUBLIC_RUST_SERVICE_URL || 'http://localhost:3001'
      const res = await fetch(`${serviceUrl}/penilaian-medis-bedah/save/${resolvedParams.no_rawat}`, {
        method: 'POST',
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-bold text-slate-500 animate-pulse uppercase tracking-widest text-xs">Memuat Data Asesmen Bedah...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex flex-col">
      <header className="bg-white/80 backdrop-blur-xl border-b border-orange-50 px-8 py-6 sticky top-0 z-30 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-4">
             <div className="w-14 h-14 bg-gradient-to-br from-orange-400 to-red-500 rounded-[1.2rem] flex items-center justify-center text-white shadow-lg shadow-orange-100">
                <Scissors className="w-7 h-7" />
             </div>
             <div>
                <div className="flex items-center gap-3">
                   <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase">ASESMEN MEDIS BEDAH</h1>
                   <span className="bg-orange-600 text-white px-3 py-1 rounded-lg text-[10px] font-black tracking-widest uppercase">SURGERY</span>
                </div>
                <p className="text-[11px] font-bold text-slate-400 flex items-center gap-3 mt-1 uppercase">
                   ...
                   <span className="w-1 h-1 bg-slate-200 rounded-full"></span>
                   <span className="text-orange-600 font-extrabold">{noRawatJoined}</span>
                </p>
             </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
           ...
           <button 
             disabled={saving}
             onClick={handleSave}
             className="bg-slate-900 text-white px-8 py-3 rounded-2xl font-black text-xs hover:bg-orange-600 shadow-xl shadow-slate-100 transition-all flex items-center gap-2 active:scale-95 disabled:opacity-50"
           >
        ...
        <div className="w-full max-w-5xl space-y-8">
          ...
          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                ...
                <select 
                  className="w-full bg-slate-50 border-2 border-slate-100/50 rounded-2xl py-4 px-4 font-bold text-slate-700 outline-none focus:border-orange-500 transition-all"
          ...
          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
              ...
              <TextAreaField label="Status Lokalis" value={formData.ket_lokalis} onChange={(v: string) => setFormData({...formData, ket_lokalis: v})} placeholder="Warna, pulsasi, tumor, dll..." />
            </div>
          </section>
          ...
          <div className="flex justify-end gap-4 pb-12">
             ...
             <button 
               disabled={saving}
               onClick={handleSave}
               className="bg-orange-600 text-white px-12 py-4 rounded-3xl font-black text-sm hover:bg-orange-700 transition-all shadow-2xl shadow-orange-100 active:scale-95 disabled:opacity-50 flex items-center gap-3"
             >
...
function InputField({ label, value, onChange, placeholder }: any) {
  return (
    <div className="space-y-3">
      ...
      <input 
        type="text"
        className="w-full bg-slate-50 border-2 border-slate-100/50 rounded-2xl py-4 px-6 font-bold text-slate-700 outline-none focus:border-orange-500 transition-all placeholder:text-slate-300"
        value={value || ''}
...
function TextAreaField({ label, value, onChange, placeholder }: any) {
  return (
    <div className="space-y-3">
      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">{label}</label>
      <textarea 
        className="w-full h-32 bg-slate-50 border-2 border-slate-100/50 rounded-[2rem] p-6 font-medium text-slate-700 outline-none focus:border-orange-500 transition-all placeholder:text-slate-200 resize-none"
        value={value || ''}
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
      />
        ...
        {['Normal', 'Abnormal', 'Tidak Diperiksa'].map((s) => (
          <button
            ...
            className={`flex-1 px-2 py-2 rounded-xl text-[9px] font-bold transition-all ${
              value === s ? 'bg-orange-600 text-white shadow-md' : 'bg-white text-slate-400 border border-slate-100 hover:bg-slate-50'
            }`}
          >
