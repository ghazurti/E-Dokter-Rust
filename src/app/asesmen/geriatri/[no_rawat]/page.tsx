...
export default function AsesmenGeriatriPage({ params }: { params: Promise<{ no_rawat: string }> }) {
  ...
  const [formData, setFormData] = useState<any>({
    ...
    integument_kebersihan: 'Normal',
    integument_warna: 'Normal',
    integument_kelembaban: 'Lembab',
    integument_gangguan_kulit: 'Normal',
    status_fungsional: '20 : Mandiri (A)',
    skrining_jatuh: 'Risiko Rendah Skor 0-5',
    status_nutrisi: 'Skor 12-14 : Status Gizi Normal',
    lainnya: '',
    lab: '',
    rad: '',
    pemeriksaan: '',
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const assessmentRes = await fetch(`${serviceUrl}/penilaian-medis-geriatri/${resolvedParams.no_rawat}`)
  ...
  const handleSave = async () => {
    ...
    try {
      const serviceUrl = process.env.NEXT_PUBLIC_RUST_SERVICE_URL || 'http://localhost:3001'
      const res = await fetch(`${serviceUrl}/penilaian-medis-geriatri/save/${resolvedParams.no_rawat}`, {
        method: 'POST',
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-bold text-slate-500 animate-pulse uppercase tracking-widest text-xs">Memuat Data Asesmen Geriatri...</p>
        </div>
      </div>
    )
  }
      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-amber-50 px-8 py-6 sticky top-0 z-30 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-4">
             <div className="w-14 h-14 bg-gradient-to-br from-amber-500 to-amber-700 rounded-[1.2rem] flex items-center justify-center text-white shadow-lg shadow-amber-100">
                <UserCircle2 className="w-7 h-7" />
             </div>
             <div>
                <div className="flex items-center gap-3">
                   <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase">ASESMEN MEDIS GERIATRI</h1>
                   <span className="bg-amber-600 text-white px-3 py-1 rounded-lg text-[10px] font-black tracking-widest uppercase">GERIATRICS</span>
                </div>
        ...
        <div className="w-full max-w-6xl space-y-8">
          ...
          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
            <h3 className="text-sm font-black text-slate-900 tracking-[0.2em] uppercase flex items-center gap-3">
              ...
              II. Status Fisik & Penilaian Khusus
            </h3>
            ...
            <div className="pt-6 border-t border-slate-50 space-y-6">
               <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Status Integument</p>
               <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  <SelectField label="Kebersihan" value={formData.integument_kebersihan} options={['Normal', 'Abnormal']} onChange={(v: string) => setFormData({...formData, integument_kebersihan: v})} />
                  <SelectField label="Warna" value={formData.integument_warna} options={['Normal', 'Pucat', 'Sianosis', 'Lain-lain']} onChange={(v: string) => setFormData({...formData, integument_warna: v})} />
                  <SelectField label="Kelembaban" value={formData.integument_kelembaban} options={['Kering', 'Lembab']} onChange={(v: string) => setFormData({...formData, integument_kelembaban: v})} />
                  <SelectField label="Gangguan Kulit" value={formData.integument_gangguan_kulit} options={['Normal', 'Rash', 'Luka', 'Memar', 'Ptekie', 'Bula']} onChange={(v: string) => setFormData({...formData, integument_gangguan_kulit: v})} />
               </div>
            </div>

            <div className="pt-6 border-t border-slate-50 space-y-6">
               <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Penilaian Fungsional & Risiko</p>
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  <SelectField label="Status Fungsional (Barthel Index)" value={formData.status_fungsional} options={['20 : Mandiri (A)', '12-19 : Ketergantungan Ringan (B)', '9-11 : Ketergantungan Sedang (B)', '5-8 : Ketergantungan Berat (C)', '0-4 : Ketergantungan Total (C)']} onChange={(v: string) => setFormData({...formData, status_fungsional: v})} />
                  <SelectField label="Skrining Risiko Jatuh" value={formData.skrining_jatuh} options={['Risiko Rendah Skor 0-5', 'Risiko Sedang Skor 6-16', 'Risiko Tinggi Skor 17-30']} onChange={(v: string) => setFormData({...formData, skrining_jatuh: v})} />
                  <SelectField label="Status Nutrisi (MNA)" value={formData.status_nutrisi} options={['Skor 12-14 : Status Gizi Normal', 'Skor 8-11 : Berisiko Malnutrisi', 'Skor 0-7 : Malnutrisi']} onChange={(v: string) => setFormData({...formData, status_nutrisi: v})} />
               </div>
            </div>
          ...
          <div className="flex justify-end gap-4 pb-12">
             ...
             <button 
               disabled={saving}
               onClick={handleSave}
               className="bg-slate-900 text-white px-12 py-4 rounded-3xl font-black text-sm hover:bg-amber-600 transition-all shadow-2xl shadow-amber-100 active:scale-95 disabled:opacity-50 flex items-center gap-3"
             >
               {saving ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <><Save className="w-5 h-5" /> SIMPAN ASESMEN GERIATRI</>}
             </button>
          </div>
        </div>
      </main>
