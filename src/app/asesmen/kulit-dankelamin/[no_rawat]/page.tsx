...
export default function AsesmenKulitKelaminPage({ params }: { params: Promise<{ no_rawat: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const assessmentRes = await fetch(`${serviceUrl}/penilaian-medis-kulitdankelamin/${resolvedParams.no_rawat}`)
          ...
          if (nurseRes.ok) {
            ...
            if (nurseData) {
              setFormData((prev: any) => ({
                ...
                statusderma: nurseData.pemeriksaan || prev.statusderma,
              }))
            }
          }
        }
  ...
  const handleAiSuggest = (data: any) => {
    setFormData((prev: any) => ({
      ...
      statusderma: data.objective || prev.statusderma,
  ...
  const handleSave = async () => {
    ...
    try {
      const serviceUrl = process.env.NEXT_PUBLIC_RUST_SERVICE_URL || 'http://localhost:3001'
      const res = await fetch(`${serviceUrl}/penilaian-medis-kulitdankelamin/save/${resolvedParams.no_rawat}`, {
        method: 'POST',
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-bold text-slate-500 animate-pulse uppercase tracking-widest text-xs">Memuat Data Asesmen Kulit & Kelamin...</p>
        </div>
      </div>
    )
  }
      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-rose-50 px-8 py-6 sticky top-0 z-30 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-4">
             <div className="w-14 h-14 bg-gradient-to-br from-rose-500 to-rose-700 rounded-[1.2rem] flex items-center justify-center text-white shadow-lg shadow-rose-100">
                <Sparkles className="w-7 h-7" />
             </div>
             <div>
                <div className="flex items-center gap-3">
                   <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase">ASESMEN MEDIS KULIT & KELAMIN</h1>
                   <span className="bg-rose-600 text-white px-3 py-1 rounded-lg text-[10px] font-black tracking-widest uppercase">DERMATOLOGY</span>
                </div>
        ...
        <div className="w-full max-w-6xl space-y-8">
          ...
          <section className={`bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8 transition-all duration-500 ${highlightActive ? 'ai-highlight' : ''}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <InputField label="Riwayat Penyakit Keluarga (RPK)" value={formData.rpk} onChange={(v: string) => setFormData({...formData, rpk: v})} />
          ...
          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
            <h3 className="text-sm font-black text-slate-900 tracking-[0.2em] uppercase flex items-center gap-3">
              ...
              II. Pemeriksaan Fisik & Dermatologis
            </h3>
            ...
            <div className="pt-6 border-t border-slate-50 space-y-6">
               <TextAreaField label="Status Dermatovenerologi (Status Lokalis)" value={formData.statusderma} onChange={(v: string) => setFormData({...formData, statusderma: v})} />
            </div>
          </section>
          ...
          <div className="flex justify-end gap-4 pb-12">
             ...
             <button 
               disabled={saving}
               onClick={handleSave}
               className="bg-slate-900 text-white px-12 py-4 rounded-3xl font-black text-sm hover:bg-rose-600 transition-all shadow-2xl shadow-rose-100 active:scale-95 disabled:opacity-50 flex items-center gap-3"
             >
               {saving ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <><Save className="w-5 h-5" /> SIMPAN ASESMEN KULIT & KELAMIN</>}
             </button>
          </div>
        </div>
      </main>
