...
export default function AsesmenNeurologiPage({ params }: { params: Promise<{ no_rawat: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const assessmentRes = await fetch(`${serviceUrl}/penilaian-medis-neurologi/${resolvedParams.no_rawat}`)
  ...
  const handleSave = async () => {
    ...
    try {
      const serviceUrl = process.env.NEXT_PUBLIC_RUST_SERVICE_URL || 'http://localhost:3001'
      const res = await fetch(`${serviceUrl}/penilaian-medis-neurologi/save/${resolvedParams.no_rawat}`, {
        method: 'POST',
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-bold text-slate-500 animate-pulse uppercase tracking-widest text-xs">Memuat Data Asesmen Neurologi...</p>
        </div>
      </div>
    )
  }
      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-indigo-50 px-8 py-6 sticky top-0 z-30 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-4">
             <div className="w-14 h-14 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-[1.2rem] flex items-center justify-center text-white shadow-lg shadow-indigo-100">
                <Brain className="w-7 h-7" />
             </div>
             <div>
                <div className="flex items-center gap-3">
                   <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase">ASESMEN MEDIS NEUROLOGI</h1>
                   <span className="bg-indigo-600 text-white px-3 py-1 rounded-lg text-[10px] font-black tracking-widest uppercase">SYARAF</span>
                </div>
        ...
        <div className="w-full max-w-5xl space-y-8">
            ...
            <div className="space-y-6 pt-6 border-t border-slate-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <SystemicToggleWithDesc 
                  ...
                  desc={formData.keterangan_kepala}
                  onChange={(v: string) => setFormData({...formData, kepala: v})}
                  onDescChange={(v: string) => setFormData({...formData, keterangan_kepala: v})}
                 />
                 <SystemicToggleWithDesc 
                  label="Thoraks" 
                  value={formData.thoraks} 
                  desc={formData.keterangan_thoraks}
                  onChange={(v: string) => setFormData({...formData, thoraks: v})}
                  onDescChange={(v: string) => setFormData({...formData, keterangan_thoraks: v})}
                 />
                 <SystemicToggleWithDesc 
                  label="Abdomen" 
                  value={formData.abdomen} 
                  desc={formData.keterangan_abdomen}
                  onChange={(v: string) => setFormData({...formData, abdomen: v})}
                  onDescChange={(v: string) => setFormData({...formData, keterangan_abdomen: v})}
                 />
                 <SystemicToggleWithDesc 
                  label="Ekstremitas" 
                  value={formData.ekstremitas} 
                  desc={formData.keterangan_ekstremitas}
                  onChange={(v: string) => setFormData({...formData, ekstremitas: v})}
                  onDescChange={(v: string) => setFormData({...formData, keterangan_ekstremitas: v})}
                 />
                 <SystemicToggleWithDesc 
                  label="Columna Vertebralis" 
                  value={formData.columna} 
                  desc={formData.keterangan_columna}
                  onChange={(v: string) => setFormData({...formData, columna: v})}
                  onDescChange={(v: string) => setFormData({...formData, keterangan_columna: v})}
                 />
                 <SystemicToggleWithDesc 
                  label="Muskuloskeletal" 
                  value={formData.muskulos} 
                  desc={formData.keterangan_muskulos}
                  onChange={(v: string) => setFormData({...formData, muskulos: v})}
                  onDescChange={(v: string) => setFormData({...formData, keterangan_muskulos: v})}
                 />
              </div>
            </div>
          ...
          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <TextAreaField label="Penunjang Lain" value={formData.penunjanglain} onChange={(v: string) => setFormData({...formData, penunjanglain: v})} />
            </div>
          ...
          <div className="flex justify-end gap-4 pb-12">
             ...
             <button 
               disabled={saving}
               onClick={handleSave}
               className="bg-indigo-600 text-white px-12 py-4 rounded-3xl font-black text-sm hover:bg-indigo-700 transition-all shadow-2xl shadow-indigo-100 active:scale-95 disabled:opacity-50 flex items-center gap-3"
             >
               {saving ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <><Activity className="w-5 h-5" /> SIMPAN ASESMEN NEUROLOGI</>}
             </button>
          </div>
        </div>
      </main>
