...
export default function AsesmenOrthopediPage({ params }: { params: Promise<{ no_rawat: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const assessmentRes = await fetch(`${serviceUrl}/penilaian-medis-orthopedi/${resolvedParams.no_rawat}`)
          ...
          if (nurseRes.ok) {
            ...
            if (nurseData) {
              setFormData((prev: any) => ({
                ...
                ket_lokalis: nurseData.pemeriksaan || prev.ket_lokalis,
              }))
            }
          }
        }
  ...
  const handleAiSuggest = (data: any) => {
    setFormData((prev: any) => ({
      ...
      ket_lokalis: data.objective || prev.ket_lokalis,
  ...
  const handleSave = async () => {
    ...
    try {
      const serviceUrl = process.env.NEXT_PUBLIC_RUST_SERVICE_URL || 'http://localhost:3001'
      const res = await fetch(`${serviceUrl}/penilaian-medis-orthopedi/save/${resolvedParams.no_rawat}`, {
        method: 'POST',
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-bold text-slate-500 animate-pulse uppercase tracking-widest text-xs">Memuat Data Asesmen Orthopedi...</p>
        </div>
      </div>
    )
  }
      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-slate-100 px-8 py-6 sticky top-0 z-30 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-4">
             <div className="w-14 h-14 bg-gradient-to-br from-slate-700 to-slate-900 rounded-[1.2rem] flex items-center justify-center text-white shadow-lg shadow-slate-100">
                <Activity className="w-7 h-7" />
             </div>
             <div>
                <div className="flex items-center gap-3">
                   <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase">ASESMEN MEDIS ORTHOPEDI</h1>
                   <span className="bg-slate-800 text-white px-3 py-1 rounded-lg text-[10px] font-black tracking-widest uppercase">ORTHOPEDICS</span>
                </div>
                <p className="text-[11px] font-bold text-slate-400 flex items-center gap-3 mt-1 uppercase">
                   ...
                   <span className="w-1 h-1 bg-slate-200 rounded-full"></span>
                   <span className="text-slate-800 font-extrabold">{noRawatJoined}</span>
                </p>
             </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
           ...
           <button 
             disabled={saving}
             onClick={handleSave}
             className="bg-slate-900 text-white px-8 py-3 rounded-2xl font-black text-xs hover:bg-slate-800 shadow-xl shadow-slate-100 transition-all flex items-center gap-2 active:scale-95 disabled:opacity-50"
           >
        ...
        <div className="w-full max-w-6xl space-y-8">
          ...
          <section className={`bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8 transition-all duration-500 ${highlightActive ? 'ai-highlight' : ''}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                ...
                <select 
                  className="w-full bg-slate-50 border-2 border-slate-100/50 rounded-2xl py-4 px-4 font-bold text-slate-700 outline-none focus:border-slate-800 transition-all"
          ...
          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
              ...
              <InputField label="Status Psikologis" value={formData.status} onChange={(v: string) => setFormData({...formData, status: v})} />
            </div>
            ...
            <div className="pt-6 border-t border-slate-50 space-y-6">
               ...
               <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                  ...
                  <SystemicToggle label="Genetalia" value={formData.genetalia} onChange={(v: string) => setFormData({...formData, genetalia: v})} />
                  <SystemicToggle label="Columna" value={formData.columna} onChange={(v: string) => setFormData({...formData, columna: v})} />
                  <SystemicToggle label="Muskulos" value={formData.muskulos} onChange={(v: string) => setFormData({...formData, muskulos: v})} />
               </div>
               <TextAreaField label="Keterangan Pemeriksaan Fisik" value={formData.lainnya} onChange={(v: string) => setFormData({...formData, lainnya: v})} />
            </div>

            <div className="pt-6 border-t border-slate-50">
               <TextAreaField label="Keterangan Lokalis (Status Lokalis Orthopedi)" value={formData.ket_lokalis} onChange={(v: string) => setFormData({...formData, ket_lokalis: v})} />
            </div>
          </section>
          ...
          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <TextAreaField label="Pemeriksaan Penunjang" value={formData.pemeriksaan} onChange={(v: string) => setFormData({...formData, pemeriksaan: v})} />
            </div>
          ...
          <div className="flex justify-end gap-4 pb-12">
             ...
             <button 
               disabled={saving}
               onClick={handleSave}
               className="bg-slate-900 text-white px-12 py-4 rounded-3xl font-black text-sm hover:bg-slate-800 transition-all shadow-2xl shadow-slate-100 active:scale-95 disabled:opacity-50 flex items-center gap-3"
             >
               {saving ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <><Save className="w-5 h-5" /> SIMPAN ASESMEN ORTHOPEDI</>}
             </button>
          </div>
        </div>
      </main>
...
function InputField({ label, value, onChange, placeholder }: any) {
  return (
    <div className="space-y-3">
      ...
      <input 
        type="text"
        className="w-full bg-slate-50 border-2 border-slate-100/50 rounded-2xl py-4 px-6 font-bold text-slate-700 outline-none focus:border-slate-900 transition-all placeholder:text-slate-300"
        value={value || ''}
...
function TextAreaField({ label, value, onChange, placeholder }: any) {
  return (
    <div className="space-y-3">
      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">{label}</label>
      <textarea 
        className="w-full h-32 bg-slate-50 border-2 border-slate-100/50 rounded-[2rem] p-6 font-medium text-slate-700 outline-none focus:border-slate-900 transition-all placeholder:text-slate-200 resize-none"
        value={value || ''}
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
      />
        ...
        {['Normal', 'Abnormal', 'Tidak Diperiksa'].map((s) => (
          <button
            ...
            className={`px-2 py-1.5 rounded-lg text-[8px] font-bold transition-all text-left ${
              value === s ? 'bg-slate-800 text-white shadow-md' : 'bg-white text-slate-400 border border-slate-100 hover:bg-slate-50'
            }`}
          >
            {s}
          </button>
