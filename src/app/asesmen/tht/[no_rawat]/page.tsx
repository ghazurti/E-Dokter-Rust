...
export default function AsesmenTHTPage({ params }: { params: Promise<{ no_rawat: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const assessmentRes = await fetch(`${serviceUrl}/penilaian-medis-tht/${resolvedParams.no_rawat}`)
          ...
          if (nurseRes.ok) {
            ...
            if (nurseData) {
              setFormData((prev: any) => ({
                ...
                kondisi: nurseData.pemeriksaan || prev.kondisi,
              }))
            }
          }
        }
  ...
  const handleSave = async () => {
    ...
    try {
      const serviceUrl = process.env.NEXT_PUBLIC_RUST_SERVICE_URL || 'http://localhost:3001'
      const res = await fetch(`${serviceUrl}/penilaian-medis-tht/save/${resolvedParams.no_rawat}`, {
        method: 'POST',
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-bold text-slate-500 animate-pulse uppercase tracking-widest text-xs">Memuat Data Asesmen THT...</p>
        </div>
      </div>
    )
  }
      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-blue-50 px-8 py-6 sticky top-0 z-30 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-4">
             <div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-cyan-600 rounded-[1.2rem] flex items-center justify-center text-white shadow-lg shadow-blue-100">
                <Ear className="w-7 h-7" />
             </div>
             <div>
                <div className="flex items-center gap-3">
                   <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase">ASESMEN MEDIS THT</h1>
                   <span className="bg-blue-600 text-white px-3 py-1 rounded-lg text-[10px] font-black tracking-widest uppercase">OTOLARINGOLOGI</span>
                </div>
        ...
        <div className="w-full max-w-5xl space-y-8">
          ...
          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
              ...
              <TextAreaField label="Status Lokalis THT" value={formData.ket_lokalis} onChange={(v: string) => setFormData({...formData, ket_lokalis: v})} placeholder="Hasil pemeriksaan telinga, hidung, tenggorokan..." />
            </div>
          </section>
          ...
          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
            ...
            <TextAreaField label="Tes Pendengaran" value={formData.tes_pendengaran} onChange={(v: string) => setFormData({...formData, tes_pendengaran: v})} />
            <TextAreaField label="Penunjang Lainnya" value={formData.penunjang} onChange={(v: string) => setFormData({...formData, penunjang: v})} />
          </section>

          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
            <h3 className="text-sm font-black text-slate-900 tracking-[0.2em] uppercase flex items-center gap-3">
              <div className="w-2 h-8 bg-emerald-500 rounded-full"></div>
              IV. Diagnosis & Perencanaan
            </h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <InputField label="Diagnosis" value={formData.diagnosis} onChange={(v: string) => setFormData({...formData, diagnosis: v})} />
              <InputField label="Diagnosis Banding" value={formData.diagnosisbanding} onChange={(v: string) => setFormData({...formData, diagnosisbanding: v})} />
            </div>
            ...
            <TextAreaField label="Tata Laksana" value={formData.tatalaksana} onChange={(v: string) => setFormData({...formData, tatalaksana: v})} />
