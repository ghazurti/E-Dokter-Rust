...
export default function AsesmenUrologiPage({ params }: { params: Promise<{ no_rawat: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const assessmentRes = await fetch(`${serviceUrl}/penilaian-medis-urologi/${resolvedParams.no_rawat}`)
  ...
  const handleSave = async () => {
    ...
    try {
      const serviceUrl = process.env.NEXT_PUBLIC_RUST_SERVICE_URL || 'http://localhost:3001'
      const res = await fetch(`${serviceUrl}/penilaian-medis-urologi/save/${resolvedParams.no_rawat}`, {
        method: 'POST',
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-bold text-slate-500 animate-pulse uppercase tracking-widest text-xs">Memuat Data Asesmen Urologi...</p>
        </div>
      </div>
    )
  }
          ...
          <div className="flex items-center gap-4">
             ...
             <div>
                <div className="flex items-center gap-3">
                   <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase">ASESMEN MEDIS UROLOGI</h1>
                   <span className="bg-indigo-600 text-white px-3 py-1 rounded-lg text-[10px] font-black tracking-widest uppercase">UROLOGY</span>
                </div>
        ...
        <div className="w-full max-w-6xl space-y-8">
          ...
          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <TextAreaField label="Riwayat Kebiasaan" value={formData.riwayat_kebiasaan} onChange={(v: string) => setFormData({...formData, riwayat_kebiasaan: v})} />
              <TextAreaField label="Riwayat Operasi Urologi" value={formData.riwayat_operasi_urologi} onChange={(v: string) => setFormData({...formData, riwayat_operasi_urologi: v})} />
            </div>
          </section>
          ...
          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
            ...
            <div className="pt-6 border-t border-slate-50 space-y-6">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  ...
                  <SystemicToggleWithDesc label="Ekstrimitas" value={formData.ekstrimitas} desc={formData.keterangan_ekstrimitas} onChange={(v: string) => setFormData({...formData, ekstrimitas: v})} onDescChange={(v: string) => setFormData({...formData, keterangan_ekstrimitas: v})} />
               </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-6 border-t border-slate-50">
               <InputField label="Nyeri Ketok CVA" value={formData.nyeri_ketok_cva} onChange={(v: string) => setFormData({...formData, nyeri_ketok_cva: v})} />
               <InputField label="Genitalia Eksternal" value={formData.genitalia_eksternal} onChange={(v: string) => setFormData({...formData, genitalia_eksternal: v})} />
               <InputField label="Colok Dubur" value={formData.colok_dubur} onChange={(v: string) => setFormData({...formData, colok_dubur: v})} />
            </div>
          ...
          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
               <TextAreaField label="Urinalisis" value={formData.urinalisis} onChange={(v: string) => setFormData({...formData, urinalisis: v})} />
               <TextAreaField label="Darah" value={formData.darah} onChange={(v: string) => setFormData({...formData, darah: v})} />
               <TextAreaField label="USG Urologi" value={formData.usg_urologi} onChange={(v: string) => setFormData({...formData, usg_urologi: v})} />
          ...
          <div className="flex justify-end gap-4 pb-12">
             ...
             >
               {saving ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <><Droplet className="w-5 h-5" /> SIMPAN ASESMEN UROLOGI</>}
             </button>
          </div>
        </div>
      </main>
